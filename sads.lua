...
local function GetAutoSize(deviceType)
    local screenSize = workspace.CurrentCamera.ViewportSize
    if deviceType == "phone" then
        return UDim2.fromOffset(math.min(screenSize.X * 0.9, 400), math.min(screenSize.Y * 0.8, 600))
    elseif deviceType == "tablet" then
        return UDim2.fromOffset(math.min(screenSize.X * 0.8, 500), math.min(screenSize.Y * 0.7, 650))
    else -- pc
        return UDim2.fromOffset(math.min(screenSize.X * 0.6, 550), math.min(screenSize.Y * 0.8, 600))
    end
end

local DeviceType = GetDeviceType()
local AutoSize = GetAutoSize(DeviceType)
...
function Library:CreateWindow(...)
	...
	if typeof(Config.Size) ~= 'UDim2' then Config.Size = AutoSize end
	...
	function Window:AddTab(Name)
		...
		local LeftSide = Library:Create('ScrollingFrame', {
			...
			BottomImage = '';
			TopImage = '';
			ScrollBarThickness = DeviceType == "phone" and 8 or 4;
			ZIndex = 2;
			Parent = TabFrame;
		});
